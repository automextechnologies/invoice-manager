        ...
        <div className="preview-content hide-scrollbar mobile-preview-adjust" style={{ 
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '5px solid #000', marginBottom: '30px', paddingBottom: '5px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', width: '50%' }}>
              <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                <img src="/logo.png" alt="Logo" style={{ maxWidth: '140px', height: 'auto' }} />
              </div>
            ...
            <tbody>
              {data.items.map((item, idx) => (
                <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                  ...
                  <td style={{ padding: '15px 8px', textAlign: 'right', fontSize: '13px' }}>{Number(item.qty || 0) > 0 ? Number(item.qty) : '-'}</td>
                  ...
                  <td style={{ padding: '15px 8px', textAlign: 'right', fontSize: '13px' }}>
                    ₹{((((Number(item.qty || 0) > 0 ? Number(item.qty) : 1) * Number(item.price || 0)) + ((((Number(item.qty || 0) > 0 ? Number(item.qty) : 1) * Number(item.price || 0)) * Number(item.gstPercent || 0)) / 100)).toFixed(2))}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '30px' }}>
            <div style={{ width: '280px' }}>
              ...
              {discountPercent > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 12px', fontSize: '14px', color: '#dc2626' }}>
